...
from scipy.stats.mstats import winsorize # ฟังก์ชันสำหรับจัดการ Outlier (Winsorization) (แม้จะไม่ได้ใช้ในตอนนี้ แต่มีไว้เพื่อบริบท)
...
st.error("⚠️ โปรดทราบ: แอปพลิเคชันนี้ออกแบบมาสำหรับชุดข้อมูลที่มีโครงสร้างคล้ายกับ `redbull_workshop_dirty.csv` เท่านั้น")
...
if uploaded_file is not None: # ถ้ามีการอัปโหลดไฟล์แล้ว
    ...
    st.success("ไฟล์ถูกอัปโหลดสำเร็จแล้ว!") # แสดงข้อความแจ้งว่าอัปโหลดสำเร็จ
    st.write("### Raw Data (5 แถวแรก)") # หัวข้อแสดงข้อมูลดิบ
    st.dataframe(df_raw.head()) # แสดง 5 แถวแรกของข้อมูลดิบ

    # --- ฟังก์ชันขั้นตอนการทำความสะอาดข้อมูล ---

    def perform_data_exploration(data): # ฟังก์ชันสำหรับสำรวจข้อมูลเบื้องต้น
        st.subheader("📊 1. Data Exploration") # หัวข้อย่อย
        st.write("#### ขนาดข้อมูล:") # หัวข้อย่อยแสดงขนาดข้อมูล
        st.write(f"แถว: {data.shape[0]:,}, คอลัมน์: {data.shape[1]}") # แสดงจำนวนแถวและคอลัมน์
        st.write("#### ข้อมูล Info:") # หัวข้อย่อยแสดงข้อมูลทั่วไปของ DataFrame
        buffer = io.StringIO()
        data.info(buf=buffer) # ดึงข้อมูล info() ไปเก็บใน buffer
        st.text(buffer.getvalue()) # แสดงผลข้อมูล info()
        st.write("#### สถิติเชิงพรรณนา:") # หัวข้อย่อยแสดงสถิติเชิงพรรณนา
    ...
    def handle_inconsistent_data(data): # ฟังก์ชันสำหรับจัดการข้อมูลที่ไม่สอดคล้องกัน
        ...
        for col in cat_cols: # วนลูปในแต่ละคอลัมน์
            unique_vals = data[col].astype(str).unique() # ดึงค่า unique ของคอลัมน์
        ...
        data['Region'] = data['Region'].astype(str).str.strip().str.lower() # แปลงเป็นตัวพิมพ์เล็กและลบช่องว่าง
        ...
        data['Product_Variant'] = data['Product_Variant'].astype(str).str.strip().str.lower()
        ...
        data['Channel'] = data['Channel'].astype(str).str.strip().str.lower()
        channel_mapping = {
        ...
        for col in cat_cols: # วนลูปในแต่ละคอลัมน์อีกครั้ง
            unique_vals = data[col].astype(str).unique() # ดึงค่า unique หลังแก้ไข
            st.write(f"**📌 {col} ({len(unique_vals)} ค่า):**") # แสดงชื่อคอลัมน์และจำนวนค่า unique
    ...
    def perform_outlier_analysis(data): # ฟังก์ชันสำหรับตรวจจับ Outlier
        ...
        # Customer_Score ถูกจัดการไปแล้วใน noisy data และคาดว่าอยู่ในช่วง 1-10 จึงลบออกจากรายการวิเคราะห์ outlier
        if 'Customer_Score' in numeric_cols: # ถ้า Customer_Score อยู่ในคอลัมน์ตัวเลข
            numeric_cols.remove('Customer_Score') # ลบออกจากรายการ
        ...
        if numeric_cols: # ถ้ามีคอลัมน์ตัวเลขให้วิเคราะห์ Outlier
            ...
            **หมายเหตุเกี่ยวกับการจัดการ Outliers:**
            ในการฝึกอบรมนี้ เราจะ **ไม่ปรับค่า Outliers** ในขั้นตอนนี้ เพื่อรักษาความถูกต้องของข้อมูลตามบริบททางธุรกิจ แม้ว่าจะมีการตรวจจับก็ตาม อย่างไรก็ตาม ในสถานการณ์จริง การจัดการ Outlier ต้องพิจารณาจากบริบทและเป้าหมายการวิเคราะห์อย่างรอบคอบ.
    ...
    if st.button("เริ่มการทำความสะอาด"): # ปุ่มสำหรับเริ่มกระบวนการ Data Cleaning
        ...
        st.markdown("---  ") # เส้นแบ่ง
        st.subheader("✅ 7. สรุปข้อมูลที่ทำความสะอาดแล้ว") # หัวข้อสรุปผล
        ...
        st.write("### ข้อมูลที่ทำความสะอาดแล้ว (5 แถวแรก)") # หัวข้อแสดงข้อมูลที่ทำความสะอาดแล้ว
        st.dataframe(df.head()) # แสดง 5 แถวแรกของข้อมูลที่ทำความสะอาดแล้ว

        # --- ดาวน์โหลดข้อมูลที่ทำความสะอาดแล้ว ---
        ...
        st.download_button( # ปุ่มสำหรับดาวน์โหลดข้อมูล
            label="ดาวน์โหลดข้อมูลที่ทำความสะอาดแล้วเป็น CSV", # ข้อความบนปุ่ม
            data=csv_buffer, # ข้อมูลที่จะให้ดาวน์โหลด
            file_name="redbull_clean.csv", # ชื่อไฟล์เมื่อดาวน์โหลด
            mime="text/csv", # ประเภทของไฟล์
            help="คลิกเพื่อดาวน์โหลดชุดข้อมูลที่ทำความสะอาดแล้ว"
        )
else: # ถ้ายังไม่ได้อัปโหลดไฟล์
    st.info("กรุณาอัปโหลดไฟล์ CSV เพื่อเริ่มการทำความสะอาดข้อมูล") # แสดงข้อความให้ผู้ใช้อัปโหลดไฟล์
